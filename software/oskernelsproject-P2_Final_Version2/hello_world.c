...
unsigned long long mythread_scheduler(unsigned long long param_list) // context pointer
{
	...
	running_thread->thread.sp = *param_ptr; // update context pointer
	running_thread->thread.fp = *(param_ptr+1);
...
void mythread_create(TCB *tcb, void *(*start_routine)(void*), int thread_id)
{
	...
	tcb->sp = tcb->context + 4000/4 - 84;

	int one = 1;

	memcpy(tcb->sp + 0, &mythread_cleanup, 4);//ra

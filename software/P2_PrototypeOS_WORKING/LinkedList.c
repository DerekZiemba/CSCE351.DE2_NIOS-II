...
TCB * PeekThread(ThreadQueue *tq, threadStatus status){
	void* ptr = LList_PeekGeneric(tq->threadQueues[status]);
	return ptr==NULL ? NULL : (TCB *) ptr;
}


TCB * LookupThread(ThreadQueue *tq, threadStatus status, int thread_id){
	node_t *node = tq->threadQueues[status]->firstNode;
	while(node!=NULL && node->data != NULL){
		node_t *selected = (((TCB*) node->data)->thread_id == thread_id) ? node : NULL;
		if(selected != NULL){
			return ((TCB *) node->data);
		}
	}
	return NULL;
}
